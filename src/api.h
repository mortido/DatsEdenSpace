...
class Api {
 public:
  explicit Api(std::string server_url, std::string token) : server_url_{std::move(server_url)},
    ...
    headers_.push_back("X-Auth-Token: " + token_);
  ...
  std::optional<std::string> get_current_round();
 ...
 private:
  ...
  int max_retries_ = 10;
  int max_rpc_ = 4;
  std::chrono::milliseconds request_interval_ = std::chrono::milliseconds(1000 / max_rpc_);
